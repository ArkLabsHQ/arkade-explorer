...
function parseArkPacket(script: Uint8Array): ParsedPacket | null {
    try {
        ...
        while (pos < buf.length) {
            ...
            extensions.push({
                type: packetType,
                name: EXTENSION_NAMES[packetType] || `Extension #${packetType}`,
            });
            if (packetType === 0) {
...
function TxTypeBadge({
    ...
    label: string;
    variant: "blue" | "purple" | "amber" | "emerald" | "orange";
}) {
    ...
    return (
        <span
            className={`inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-full border ${styles[variant]}`}
        >
            {label}
        </span>
    );
        ...
        <div className="space-y-1.5">
            ...
            {txids.length > 5 && (
                <button
                    ...
                    aria-label={
                        expanded
                            ? "Show fewer forfeit transactions"
                            : `Show all ${txids.length} forfeit transactions`
                    }
                    className="text-xs text-primary hover:text-primary/80 transition-colors duration-200 active:scale-[0.97]"
                >
...
function OutputCard({
    ...
    const effectiveVtxo =
        checkpointVtxo && !output.isAnchor
            ? checkpointVtxo
            : forfeitVtxo && output.isForfeit
              ? forfeitVtxo
              : output.vtxo;
    const vtxo = effectiveVtxo;
    const vtxoStatus = vtxo ? deriveVtxoStatus(vtxo, pendingOutpoints) : undefined;
    const isSpent = vtxo?.isSpent === true || (vtxo?.spentBy && vtxo.spentBy !== "");
    ...
    if (!output.isAnchor && !output.isArkExtension) {
        if (vtxo?.assets && vtxo.assets.length > 0) {
            vtxo.assets.forEach((a: { assetId: string; amount: number }) => outputAssets.push(a));
        } else if (assetPacket) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            assetPacket.groups?.forEach((g: any, gi: number) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        <div className="flex items-center gap-2">
            ...
            >
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        {output.isBatch && output.batchInfo && (
                            <Link
                                ...
                                title={
                                    batchRootTxid
                                        ? `View batch root tx: ${batchRootTxid}`
                                        : undefined
                                }
                            >
                ...
                {output.isForfeit ? (
                    ...
                    <div className="flex items-center gap-1.5">
                        {output.arkAddress.startsWith("ark1") ||
                        output.arkAddress.startsWith("tark1") ? (
                            <Link
            ...
            <div className="w-7 flex items-center justify-center shrink-0">
                {isSpent && spendingTxid ? (
                    <Link
                        to={
                            spendingIsCommitment
                                ? `/commitment-tx/${spendingTxid}`
                                : `/tx/${spendingTxid}`
                        }
                        className="flex items-center justify-center w-7 h-7 rounded-full bg-primary text-primary-foreground hover:bg-primary/80 transition-colors duration-200 active:scale-[0.97]"
                ...
                <div>
                    ...
                    <div className="ml-2 space-y-0.5 mt-0.5">
                        ...
                        {inputs.hiddenCount > 0 && (
                            ...
                            >
                                Show {inputs.hiddenCount} more input
                                {inputs.hiddenCount !== 1 ? "s" : ""}
                            </button>
                        )}
                    </div>
                </div>
                    ...
                    <div className="ml-2 space-y-0.5 mt-0.5">
                        ...
                        {outputs.hiddenCount > 0 && (
                            <button
                                ...
                                className="text-xs text-primary hover:underline mt-1"
                            >
                                Show {outputs.hiddenCount} more output
                                {outputs.hiddenCount !== 1 ? "s" : ""}
                            </button>
                        )}
                    </div>
                </div>
...
export function TransactionDetail({
    ...
    const { parsedTx, subtype, inputs, outputs, assetPacket, packetExtensions, forfeitAddress } =
        useMemo(() => {
            ...
            if (type === "arkade" && arkadeData?.hex) {
                try {
                    const psbtBytes = Uint8Array.from(atob(arkadeData.hex), (c) => c.charCodeAt(0));
            ...
            if (detectedSubtype === "generic" && parsed && serverInfo?.checkpointTapscript) {
                try {
            ...
            if (detectedSubtype === "generic" && parsed) {
                try {
                    const cosignerFields = getArkPsbtFields(parsed, 0, CosignerPublicKey);
            ...
            for (let i = 0; i < parsed.outputsLength; i++) {
                ...
                const batchInfo =
                    type === "commitment" ? commitmentData?.metadata?.batches?.[batchKey] : null;
                const isBatch =
    ...
    const batchVouts =
        type === "commitment" && commitmentData?.metadata?.batches
    ...
    const rootConnectorTxid =
        type === "commitment" && commitmentData?.connectors?.length
            ? (() => {
        ...
        if (rootConnectorData && typeof rootConnectorData === "string") {
            try {
                ...
                if (isHexData) {
                    ...
                    const psbtBytes = Uint8Array.from(atob(rootConnectorData), (c) =>
                        c.charCodeAt(0),
                    );
    ...
    const { earliestCreatedAt, earliestExpiry, aggregateKind } = useMemo(() => {
        if (type !== "arkade" || !vtxoData || vtxoData.length === 0) {
            return {
                ...
                earliestExpiry: null,
                aggregateKind: "active" as ExpiryKind,
            };
        }
        ...
        for (const vtxo of vtxoData) {
            ...
            if (vtxo.createdAt) {
                const ts =
                    vtxo.createdAt instanceof Date
        ...
        <div className="space-y-6">
            ...
            <div className={`rounded-xl border border-border bg-card p-6 ${CARD_SHADOW}`}>
                    ...
                    <div className="space-y-6">
                        <dl className="divide-y-0">
                            ...
                            <InfoRow
                                ...
                                value={
                                    commitmentData.metadata.totalInputVtxos?.toLocaleString() ?? "0"
                                }
                            />
                            <InfoRow
                ...
                {type === "arkade" && (earliestCreatedAt || earliestExpiry) && (
                    <dl className="divide-y-0 mt-6">
                        {earliestCreatedAt && (
                            <InfoRow label="Created" value={formatTimestamp(earliestCreatedAt)} />
                        )}
                        {aggregateKind === "active" ? (
                            earliestExpiry && (
                                <InfoRow label="Expires" value={formatTimestamp(earliestExpiry)} />
                            )
                        ) : (
                            <InfoRow label="Status" value={expiryKindLabel(aggregateKind)} />
                        )}
                    </dl>
                )}
            ...
            {parsedTx && (inputs.length > 0 || outputs.length > 0) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Inputs */}
                    <div
                        className={`rounded-xl border border-border bg-card p-5 overflow-x-auto ${CARD_SHADOW}`}
                    >
                        ...
                        <div className="space-y-2">
                            {inputs.map((input) => {
                                ...
                                const inputMempoolUrl =
                                    type === "commitment" && input.txid
...
function RawDataSection({
    ...
    const label = type === "arkade" ? "Raw virtual transaction" : "Raw transaction";
