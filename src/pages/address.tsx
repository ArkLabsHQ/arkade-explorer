...
export function AddressPage() {
    ...
    const allVtxos = useMemo(() => data?.pages.flatMap((page) => page.vtxos) ?? [], [data]);
    ...
    const filteredVtxos = useMemo(() => {
        ...
        return allVtxos.filter((vtxo) => vtxo.virtualStatus.state === statusFilter);
    }, [allVtxos, statusFilter]);

    const filteredBalance = useMemo(() => sumVtxoValue(filteredVtxos), [filteredVtxos]);
        ...
        <PageTransition>
            <div className="space-y-6">
                ...
                >
                    ...
                    {!isLoading && !error && allVtxos.length > 0 && (
                        <AddressStats
                            ...
                            className={hasAssets ? "flex-1 min-w-0" : undefined}
                        />
                    )}
                </div>
                ...
                {isLoading ? (
                    ...
                    <ErrorMessage
                        message={error instanceof Error ? error.message : "Failed to fetch VTXOs"}
                    />
                ) : (
                    <>
                        <VtxoList
